...
func (c *PermController) GetPermission(ctx iris.Context) {
	...
	_, _ = ctx.JSON(agentUtils.ApiRes(200, "操作成功", c.PermRepo.PermTransform(perm)))
}
...
func (c *PermController) CreatePermission(ctx iris.Context) {
	...
	if err != nil {
		_, _ = ctx.JSON(agentUtils.ApiRes(400, fmt.Sprintf("Error create prem: %s", err.Error()), nil))
		return
	}

	if perm.ID == 0 {
		_, _ = ctx.JSON(agentUtils.ApiRes(400, "操作失败", perm))
		return
	}
	_, _ = ctx.JSON(agentUtils.ApiRes(200, "操作成功", c.PermRepo.PermTransform(perm)))
...
func (c *PermController) UpdatePermission(ctx iris.Context) {
	...
	if err != nil {
		_, _ = ctx.JSON(agentUtils.ApiRes(400, fmt.Sprintf("Error update prem: %s", err.Error()), nil))
		return
	}

	_, _ = ctx.JSON(agentUtils.ApiRes(200, "操作成功", c.PermRepo.PermTransform(aul)))
